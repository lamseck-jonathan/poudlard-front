<template>
    <q-card class="q-px-md" style="width:650px;height:550px">
        <q-item>
        ...
        <q-separator/>
        <q-card-section class="flex justify-center items-center"  style="height:440px">
            <div  v-if="test?.test_type === 'QuestionChoixUnique'" >
                <RadioUniqueAnswer 
                v-model="uniqueAnswer" 
                :answers="test.choix"
                />
            </div>
            <div v-if="test?.test_type === 'QuestionChoixMultiple'" >
                <RadioMultipleAnswer v-model="selectedAnswers" 
                :answers="test.choix"
                />
            </div>
            <div style="width:500px;height:900px" 
            v-if="test?.test_type === 'QuestionReponse'" 
            >
                <q-input
                    v-model="textAnswer"
                    filled
